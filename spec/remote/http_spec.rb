...
RSpec.describe ClaudeInbox::Remote::Http do
  ...
  describe "reading a request head" do
    subject(:request) { described_class.read_head(StringIO.new(text.b), deadline: deadline) }

    context "with a request line and headers" do
      let(:text) { "POST /api/sessions?x=1 HTTP/1.1\r\nHost: 127.0.0.1:7433\r\nContent-Type:  application/json \r\n\r\n" }
    ...
    context "with bare newlines" do
      let(:text) { "GET / HTTP/1.0\nHost: x\n\n" }

      it "takes a bare newline as a line end" do
        expect(request.headers).to eq({"host" => "x"})
      end
    end

    context "with Content-Length sent twice" do
    ...
    it "refuses a line over 8 KiB, more than 64 headers, and anything that isn't HTTP/1.x" do
      [
        ...
        expect { described_class.read_head(StringIO.new(text), deadline: deadline) }.to be_http_refused_with(400)
      end
      longest = described_class.read_head(StringIO.new("GET / HTTP/1.1\r\nX-Long: #{"a" * (8192 - 9)}\r\n\r\n"), deadline: deadline)
      expect(longest.headers["x-long"].bytesize).to eq(8183)
      most = described_class.read_head(StringIO.new("GET / HTTP/1.1\r\n" + "X-A: 1\r\n" * 64 + "\r\n"), deadline: deadline)
      expect(most.headers["x-a"]).to match(/\A1(, 1){63}\z/)
    end
    ...
    it "wants a Content-Length within the limit, and nothing chunked" do
      {
        ...
        request = described_class.read_head(StringIO.new(text), deadline: deadline)
