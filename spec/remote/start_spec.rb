...
RSpec.describe ClaudeInbox::Remote::Start do
  ...
  describe "GET /api/options" do
    it "offers the choices, only the permission modes a phone may use, and the directories" do
      older = File.join(tmp, "code", "older").tap { |dir| FileUtils.mkdir_p(dir) }
      tree = File.join(tmp, "code", "tree", ".claude", "worktrees", "wip").tap { |dir| FileUtils.mkdir_p(dir) }
      store.update([
      ...
      body = request(remote_start, "GET", "/api/options").json
    ...
    it "labels a directory by as many trailing names as it takes to tell it apart" do
      ...
      expect(request(remote_start, "GET", "/api/options").json["dirs"].map { |d| d["label"] }).to eq(%w[code/app a/x/app b/x/app])
    end

    it "says when it is a fixture, so a test inbox can't pass for a real one" do
      options[:fixture] = true
      expect(request(remote_start, "GET", "/api/options").json["fixture"]).to be(true)
    end
  end

  describe "POST /api/sessions" do
    it "starts the session the form would, and tells the inbox without moving anything" do
      r = request(remote_start, "POST", "/api/sessions", {prompt: "fix it", cwd: "app", name: "phone", model: "opus", remote: true})
      ...
      expect(drain(queue)).to eq([[:notice, "remote: starting session…"], [:remote_started, "deadbeef", "192.168.1.30"]])
    ...
    it "hands back the claude.ai/code page once the session has registered its bridge" do
      ...
      expect(request(remote_start, "POST", "/api/sessions", {prompt: "go", cwd: project, remote: true}).json["url"]).to eq("https://claude.ai/code/session_01AbC")
    end
    ...
    it "answers without waiting for a bridge a session without Remote Control seldom registers" do
      ...
      r = request(remote_start, "POST", "/api/sessions", {prompt: "go", cwd: project, remote: false})
      expect(r.status).to eq(201)
    ...
    it "still hands back a bridge that is already there without Remote Control, looking once" do
      ...
      options[:bridge_wait] = 3
      began = Process.clock_gettime(Process::CLOCK_MONOTONIC)
      expect(request(remote_start, "POST", "/api/sessions", {prompt: "go", cwd: project, remote: false}).json["url"]).to eq("https://claude.ai/code/session_01AbC")
      expect(Process.clock_gettime(Process::CLOCK_MONOTONIC) - began).to be < 1
    end
    ...
    it "refuses what the form refuses, naming the field, and starts nothing" do
      {
        ...
        r = request(remote_start, "POST", "/api/sessions", params)
    ...
    it "caps the permission mode after working out what default means in that directory" do
      expect(request(remote_start, "POST", "/api/sessions", {prompt: "go", cwd: project, permission_mode: "bypassPermissions"}).json).to eq(
      ...
      expect(request(remote_start, "POST", "/api/sessions", {prompt: "go", cwd: project}).status).to eq(403)
      settings[project] = ClaudeInbox::Settings::Defaults.new(nil, nil, "plan")
      expect(request(remote_start, "POST", "/api/sessions", {prompt: "go", cwd: project}).status).to eq(201)
    ...
    it "follows /config's Remote Control setting when the request leaves remote out, as the form does" do
      ...
      request(remote_start, "POST", "/api/sessions", {prompt: "go", cwd: project})
      request(remote_start, "POST", "/api/sessions", {prompt: "go", cwd: project, remote: false})
    ...
    it "takes the wider list it was given" do
      ...
      expect(request(remote_start, "POST", "/api/sessions", {prompt: "go", cwd: project, permission_mode: "acceptEdits"}).status).to eq(201)
      expect(request(remote_start, "GET", "/api/options").json["permission_modes"]).to eq(%w[default acceptEdits plan])
    end

    it "wants a JSON object, sized up front" do
      expect(request(remote_start, "POST", "/api/sessions", {prompt: "go"}, type: "text/plain").status).to eq(415)
      expect(request(remote_start, "POST", "/api/sessions", {prompt: "go"}, type: "application/json; charset=utf-8").status).to eq(422)
      expect(request(remote_start, "POST", "/api/sessions", "[1]").status).to eq(400)
      expect(request(remote_start, "POST", "/api/sessions", "{").status).to eq(400)
      expect(request(remote_start, "POST", "/api/sessions", "{\"prompt\": \"\xFF\"}".b).json["error"]).to eq("the body isn't UTF-8")
      expect(request_raw(remote_start, "POST /api/sessions HTTP/1.1\r\nContent-Type: application/json\r\n\r\n").status).to eq(411)
      expect(request(remote_start, "POST", "/api/sessions", "{}", headers: {"Transfer-Encoding" => "chunked"}).status).to eq(400)
      too_big = request_raw(remote_start, "POST /api/sessions HTTP/1.1\r\n" \
    ...
    it "saves the images only once the request passes, and points the prompt at them" do
      data = [PNG].pack("m0")
      expect(request(remote_start, "POST", "/api/sessions", {prompt: "", cwd: project, images: [{data: data}]}).status).to eq(422)
      expect(Dir.exist?(File.join(tmp, "images"))).to be(false)

      expect(request(remote_start, "POST", "/api/sessions", {prompt: "like [Image #2], not [Image #1]", cwd: project, images: [{data: data}, {data: data}]}).status).to eq(201)
    ...
    it "refuses more than eight images, or one that isn't an image, saying which" do
      ...
      expect(request(remote_start, "POST", "/api/sessions", {prompt: "go", cwd: project, images: [png] * 9}).status).to eq(413)
      r = request(remote_start, "POST", "/api/sessions", {prompt: "go", cwd: project, images: [png, {data: ["hello"].pack("m0")}]})
      expect([r.status, r.json]).to eq([415, {"error" => "image 2 isn't a PNG, JPEG, GIF or WebP image", "field" => "images", "index" => 1}])
      expect(request(remote_start, "POST", "/api/sessions", {prompt: "go", cwd: project, images: [{data: "!!not base64"}]}).json["index"]).to eq(0)
      expect(request(remote_start, "POST", "/api/sessions", {prompt: "go", cwd: project, images: ["x"]}).status).to eq(415)
      expect(request(remote_start, "POST", "/api/sessions", {prompt: "go", cwd: project, images: "x"}).status).to eq(422)
      expect(client.spawns).to be_empty
    end

    it "answers a repeated Idempotency-Key with the first answer instead of a second session" do
      first = request(remote_start, "POST", "/api/sessions", {prompt: "go", cwd: project}, headers: {"Idempotency-Key" => "k1"})
      again = request(remote_start, "POST", "/api/sessions", {prompt: "go", cwd: project}, headers: {"Idempotency-Key" => "k1"})
      expect([first.status, again.status]).to eq([201, 200])
      expect(again.json).to eq(first.json)
      expect(client.spawns.size).to eq(1)
      expect(request(remote_start, "POST", "/api/sessions", {prompt: "go", cwd: project}, headers: {"Idempotency-Key" => "k2"}).status).to eq(201)
    end

    it "refuses a key sent again with a different request, rather than answer it with the first start" do
      expect(request(remote_start, "POST", "/api/sessions", {prompt: "first task", cwd: project}, headers: {"Idempotency-Key" => "k1"}).status).to eq(201)
      r = request(remote_start, "POST", "/api/sessions", {prompt: "second task", cwd: project}, headers: {"Idempotency-Key" => "k1"})
      ...
      expect(request(remote_start, "POST", "/api/sessions", {prompt: "first task", cwd: project}, headers: {"Idempotency-Key" => "k1"}).status).to eq(200)
    end

    it "takes an empty Idempotency-Key as none, so every start is a new one" do
      2.times { expect(request(remote_start, "POST", "/api/sessions", {prompt: "go", cwd: project}, headers: {"Idempotency-Key" => ""}).status).to eq(201) }
      expect(client.spawns.size).to eq(2)
    end

    it "says 409 while the first request with that key is still starting" do
      client.hold
      first = Thread.new { request(remote_start, "POST", "/api/sessions", {prompt: "go", cwd: project}, headers: {"Idempotency-Key" => "k1"}) }
      expect(wait_for { client.spawns.size == 1 }).to be(true)
      expect(request(remote_start, "POST", "/api/sessions", {prompt: "go", cwd: project}, headers: {"Idempotency-Key" => "k1"}).json).to eq({"error" => "already starting"})
      client.release
    ...
    it "passes the CLI's refusal on as a 500, and lets the same key try again" do
      ...
      r = request(remote_start, "POST", "/api/sessions", {prompt: "go", cwd: project}, headers: {"Idempotency-Key" => "k1"})
      expect([r.status, r.json]).to eq([500, {"error" => "claude --bg failed: Workspace not trusted", "source" => "claude"}])
      expect(drain(queue).last).to eq([:notice, "remote start failed: claude --bg failed: Workspace not trusted"])
      expect(r.note).to eq("claude --bg failed: Workspace not trusted")
      client.fail_spawn(nil)
      expect(request(remote_start, "POST", "/api/sessions", {prompt: "go", cwd: project}, headers: {"Idempotency-Key" => "k1"}).status).to eq(201)
    end

    it "says so under --fixture, on the answer as well" do
      options[:fixture] = true
      expect(request(remote_start, "POST", "/api/sessions", {prompt: "go", cwd: project}).json["fixture"]).to be(true)
    end
  end
end
