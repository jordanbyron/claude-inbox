...
RSpec.describe ClaudeInbox::Remote::Start do
  ...
  let(:options) { {} }
  let(:phone) do
    StartPhone.new(described_class.new(client: client, store: store, queue: queue,
  ...
  describe "GET /api/options" do
    it "offers the choices, only the permission modes a phone may use, and the directories" do
      older, tree, kept = [%w[code older], %w[code tree .claude worktrees wip], %w[code trusted]].map { |parts| File.join(tmp, *parts) }
      FileUtils.mkdir_p([older, tree, kept])
      store.update([
      ...
      trusted.replace([kept, project, File.join(tmp, "gone")])
  ...
  describe "POST /api/sessions" do
    it "starts the session the form would, and tells the inbox without moving anything" do
      r = phone.start({prompt: "fix it", cwd: "app", name: "phone", model: "opus", remote: true})
      ...
      expect(Array.new(queue.size) { queue.pop }).to eq([[:notice, "remote: starting session…"], [:remote_started, "deadbeef", "192.168.1.30"]])
    ...
    it "hands back the claude.ai/code page once the session has registered its bridge" do
      ...
      expect(phone.start({prompt: "go", cwd: project, remote: true}).json["url"]).to eq("https://claude.ai/code/session_01AbC")
    end

    context "when it would wait seconds for a bridge" do
      let(:options) { {bridge_wait: 3} }

      it "answers without waiting for a bridge a session without Remote Control seldom registers" do
        began = Process.clock_gettime(Process::CLOCK_MONOTONIC)
        r = phone.start({prompt: "go", cwd: project, remote: false})
        expect(r.status).to eq(201)
      ...
      it "still hands back a bridge that is already there without Remote Control, looking once" do
        ...
        File.write(File.join(tmp, "jobs", "deadbeef", "state.json"), JSON.generate(bridgeSessionId: "cse_01AbC"))
        began = Process.clock_gettime(Process::CLOCK_MONOTONIC)
        expect(phone.start({prompt: "go", cwd: project, remote: false}).json["url"]).to eq("https://claude.ai/code/session_01AbC")
        expect(Process.clock_gettime(Process::CLOCK_MONOTONIC) - began).to be < 1
      end
    end
    ...
    it "caps the permission mode after working out what default means in that directory" do
      ...
      expect(phone.start({prompt: "go", cwd: project}).status).to eq(403)
      settings[project] = ClaudeInbox::Settings::Defaults.new(nil, nil, "plan")
      expect(phone.start({prompt: "go", cwd: project}).status).to eq(201)
    ...
    it "follows /config's Remote Control setting when the request leaves remote out, as the form does" do
      ...
      phone.start({prompt: "go", cwd: project, remote: false})
    ...
    context "when auto isn't an allowed mode" do
      let(:options) { {allowed_modes: %w[default plan]} }
    ...
    context "given a wider list of modes" do
      let(:options) { {allowed_modes: %w[default plan acceptEdits]} }

      it "takes the wider list it was given" do
        expect(phone.start({prompt: "go", cwd: project, permission_mode: "acceptEdits"}).status).to eq(201)
        expect(phone.get("/api/options").json["permission_modes"]).to eq(%w[default acceptEdits plan])
      end
    end

    it "wants a JSON object, sized up front" do
      expect(phone.start({prompt: "go"}, type: "text/plain").status).to eq(415)
      expect(phone.start({prompt: "go"}, type: "application/json; charset=utf-8").status).to eq(422)
      expect(phone.start("[1]").status).to eq(400)
      expect(phone.start("{").status).to eq(400)
      expect(phone.start("{\"prompt\": \"\xFF\"}".b).json["error"]).to eq("the body isn't UTF-8")
      expect(phone.send_raw("POST /api/sessions HTTP/1.1\r\nContent-Type: application/json\r\n\r\n").status).to eq(411)
      expect(phone.start("{}", headers: {"Transfer-Encoding" => "chunked"}).status).to eq(400)
      too_big = phone.send_raw("POST /api/sessions HTTP/1.1\r\n" \
    ...
    it "saves the images only once the request passes, and points the prompt at them" do
      data = [PNG].pack("m0")
      expect(phone.start({prompt: "", cwd: project, images: [{data: data}]}).status).to eq(422)
      expect(Dir.exist?(File.join(tmp, "images"))).to be(false)

      expect(phone.start({prompt: "like [Image #2], not [Image #1]", cwd: project, images: [{data: data}, {data: data}]}).status).to eq(201)
    ...
    it "refuses more than eight images, or one that isn't an image, saying which" do
      ...
      expect(phone.start({prompt: "go", cwd: project, images: [png] * 9}).status).to eq(413)
      r = phone.start({prompt: "go", cwd: project, images: [png, {data: ["hello"].pack("m0")}]})
      expect([r.status, r.json]).to eq([415, {"error" => "image 2 isn't a PNG, JPEG, GIF or WebP image", "field" => "images", "index" => 1}])
      expect(phone.start({prompt: "go", cwd: project, images: [{data: "!!not base64"}]}).json["index"]).to eq(0)
      expect(phone.start({prompt: "go", cwd: project, images: ["x"]}).status).to eq(415)
      expect(phone.start({prompt: "go", cwd: project, images: "x"}).status).to eq(422)
      expect(client.spawns).to be_empty
    end

    it "answers a repeated Idempotency-Key with the first answer instead of a second session" do
      first = phone.start({prompt: "go", cwd: project}, headers: {"Idempotency-Key" => "k1"})
      again = phone.start({prompt: "go", cwd: project}, headers: {"Idempotency-Key" => "k1"})
      expect([first.status, again.status]).to eq([201, 200])
      expect(again.json).to eq(first.json)
      expect(client.spawns.size).to eq(1)
      expect(phone.start({prompt: "go", cwd: project}, headers: {"Idempotency-Key" => "k2"}).status).to eq(201)
    end

    it "refuses a key sent again with a different request, rather than answer it with the first start" do
      expect(phone.start({prompt: "first task", cwd: project}, headers: {"Idempotency-Key" => "k1"}).status).to eq(201)
      r = phone.start({prompt: "second task", cwd: project}, headers: {"Idempotency-Key" => "k1"})
      ...
      expect(phone.start({prompt: "first task", cwd: project}, headers: {"Idempotency-Key" => "k1"}).status).to eq(200)
    end

    it "takes an empty Idempotency-Key as none, so every start is a new one" do
      2.times { expect(phone.start({prompt: "go", cwd: project}, headers: {"Idempotency-Key" => ""}).status).to eq(201) }
      expect(client.spawns.size).to eq(2)
    end

    it "says 409 while the first request with that key is still starting" do
      client.hold
      first = Thread.new { phone.start({prompt: "go", cwd: project}, headers: {"Idempotency-Key" => "k1"}) }
      expect(wait_for { client.spawns.size == 1 }).to be(true)
      expect(phone.start({prompt: "go", cwd: project}, headers: {"Idempotency-Key" => "k1"}).json).to eq({"error" => "already starting"})
      client.release
    ...
    it "passes the CLI's refusal on as a 500, and lets the same key try again" do
      ...
      r = phone.start({prompt: "go", cwd: project}, headers: {"Idempotency-Key" => "k1"})
      expect([r.status, r.json]).to eq([500, {"error" => "claude --bg failed: Workspace not trusted", "source" => "claude"}])
      expect(Array.new(queue.size) { queue.pop }.last).to eq([:notice, "remote start failed: claude --bg failed: Workspace not trusted"])
      expect(r.note).to eq("claude --bg failed: Workspace not trusted")
      client.fail_spawn(nil)
      expect(phone.start({prompt: "go", cwd: project}, headers: {"Idempotency-Key" => "k1"}).status).to eq(201)
    end
    ...
    context "under --fixture" do
      ...
      it "says so under --fixture, on the answer as well" do
        expect(phone.start({prompt: "go", cwd: project}).json["fixture"]).to be(true)
      end
    end
  end
end
