...
- Tests are RSpec (`RSpec.describe` / `it` / `expect(x).to eq`) in
  ...
  and no top-level constant, and a spec builds its world with `let`,
  `subject`, `before` and nested `context`s, not helper modules.
  `spec/support` holds only fakes that stand in for collaborators and
  custom matchers; no group is tagged to pull in helpers.
