...
module ClaudeInbox
  module Remote
    ...
    # AgentsClient#spawn the `n` form makes, and pairs phones with it. Like
    # the Poller it reaches App only through the queue, and it never writes
    # to the terminal: every failure becomes a response or a notice.
    class Listener
      DEFAULT_PORT = 7433
      ...
      def in_background
        Thread.new do
          ...
          @queue << [:notice, printable(e.message)]
        end
      end
